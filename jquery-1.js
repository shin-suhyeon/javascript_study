// 제이쿼리 시작
// 자바스크립트 DOM 선택 또는 제어를 쉽게 도와주는 라이브러리 언어
// 제이쿼리 사용을 위해서는 cdnJS 설치파일을 링크로 연결해야 사용할 수 있음(필수!~!)
...
const $btn = $('#btn');
const $btn2 = $('#btn2');
$btn.on('click',function(){ $('h1+p').css('background-color','lime')})
$btn2.on('click',function(){
    $('img').attr('src','./project/casper/img/advantage2.png')
    //자바스크립트객체.src, = '경로'
})

const $p = $('p')
console.log($p.length)//3
//반복문(for)없이도 변수 하나로 전체를 인식한당~~~!!!!!~!!~!~!!!!~!~~!~!~!~!~~!~!~!~!~!~
$p.css('border','1px solid black')

const p = document.querySelectorAll('p')
for(let i of p){i.style.border = '2px solid red'}

$p.eq(1).css('border','5px solid black')
$p.filter('.first').css('transform','scale(1.2)')
