...
* 배열 데이터는 0 부터 데이터를 인식한다.
* 2번째 데이터를 출력하고 싶으면? 배열번호는 -1해서 1번
* 배열의 데이터 번호 == index
* `배열명[index]`
* 배열의 `length` 속성은 inde와 다르게 1부터 인식한다.
* `배열.length` `객체.속성`
### 배열의 추가 속성
...
* `innerText` : 객체.속성
* `<span>0</span>` span 태그가 아닌 단순 문자 0 만 변경하려고 할 때 사용하는 속성
## 자바스크립트 연산자
* `=` 대입연산자
